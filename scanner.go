...
func scanToCache(t reflect.Type, cache map[reflect.Type]Codec) (Codec, error) {
	...
	c, err := scanTypeWithBothTypes(t, tinyT)
	if err != nil {
		return nil, err
	}
...
func scanWithTinyReflect(t *tinyreflect.Type) (Codec, error) {
	// For now, disable caching for tinyreflect to avoid conflicts
	// TODO: Implement proper caching using tinyreflect.Type as key
...
// scanTypeWithBothTypes scans the type using both reflect.Type and tinyreflect.Type
// This is a transitional function to maintain compatibility while migrating
func scanTypeWithBothTypes(reflectT reflect.Type, tinyT *tinyreflect.Type) (Codec, error) {
	kind := tinyT.Kind()
	switch kind {
	case K.Pointer:
		// For pointers, we need to get the element type
		elem, err := getElementType(tinyT)
		if err != nil {
			return nil, err
		}
	...
	case K.Slice:
		...
		case K.Pointer:
			...
			// Use the original reflect.Type for element creation
			return &reflectSliceOfPtrCodec{
				elemType:  reflectT.Elem().Elem(), // Use original reflect.Type
				elemCodec: elemCodec,
			}, nil
		default:
			elemCodec, err := scanTypeWithTinyReflect(elem)
	...
	return nil, Err(D.Type, D.Binary, tinyT.String(), D.Not, D.Supported)
}

// scanTypeWithTinyReflect scans the type using only tinyreflect.Type
func scanTypeWithTinyReflect(t *tinyreflect.Type) (Codec, error) {
	...
	case K.Slice:
		...
		case K.Int, K.Int8, K.Int16, K.Int32, K.Int64:
			return new(varintSliceCodec), nil
		case K.Pointer:
			// For slice of pointers, we need to defer to the version that has reflect.Type
			return nil, Err(D.Type, "slice of pointers", D.Not, D.Supported, "without reflect.Type")
		default:
			elemCodec, err := scanTypeWithTinyReflect(elem)
			if err != nil {
				return nil, err
